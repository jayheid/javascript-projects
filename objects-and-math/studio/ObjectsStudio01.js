...
function buildCrewArray (idSelection, arr){
  let crewArray = [];
  for (let i = 0; i < idSelection.length; i++){
    for (let j in arr){
      if (arr[j].astronautID === idSelection[i]){
        crewArray.push(arr[j]);
      }
    }
  }
  return crewArray;
...
selectedCrew = buildCrewArray(selectedNums, animals)
