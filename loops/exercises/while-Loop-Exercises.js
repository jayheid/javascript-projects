...
//  fuelLevel = input.question("Provide a fuel level");
...
while (astronautNumber < 1 || astronautNumber > 7 || isNaN(astronautNumber)) {
  astronautNumber = input.question("Provide a number of astronauts: ");
}
  
//c. Use a final loop to monitor the fuel status and the altitude of the shuttle. Each iteration, decrease the fuel level by 100 units for each astronaut aboard. Also, increase the altitude by 50 kilometers.
fuelUsage = astronautNumber * 100;

while (fuelLevel > fuelUsage){
  fuelLevel -= fuelUsage;
  shuttleAltitude += 50;
}
